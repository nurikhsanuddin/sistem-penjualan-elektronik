								...
								<td>
									<?php if ($value->status_bayar == 1) { ?>
										...
										<button class="btn btn-sm btn-flat btn-primary" data-toggle="modal"
											data-target="#kirim<?= $value->id_transaksi ?>"><i class="fa fa-paper-plane"></i>
											Kirim</button>
									<?php } ?>
					...
					<table class="table" id="example1">
						...
						<?php foreach ($pesanan_selesai as $key => $value) { ?>
							<tr>
								...
								<td>
									<b>Rp.<?= number_format($value->total_bayar, 0) ?></b><br>
									<?php if (is_numeric($value->id_pelanggan)) { ?>
										<span class="badge badge-success">Diterima/Sampai</span>
									<?php } else { ?>
										<span class="badge badge-warning">Oleh <?= $value->id_pelanggan ?></span>
									<?php } ?>
								</td>
								<td>
									<p><?= $value->keterangan ?></p>
				...
				<div class="modal-body">
					...
					<table class="table">
						...
						<tr>
							<th>Resi</th>
							<th>:</th>
							<th><input name="keterangan" class="form-control" placeholder="Resi"></th>
						</tr>
					</table>
